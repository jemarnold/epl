...
test_that("replace_outliers() detects and replaces outliers with NA", {
    x <- c(1:10, 100, 11:20)
    result <- replace_outliers(x, 3, method = "NA")
...
test_that("replace_outliers() respects t0 threshold", {
    ...
    strict <- replace_outliers(x, 3, t0 = 1)
    lenient <- replace_outliers(x, 3, t0 = 5)
...
test_that("replace_outliers() validates inputs correctly", {
    ...
    expect_error(replace_outliers("text", 3), "x.*?numeric")  # non-numeric x
    expect_error(replace_outliers(x, -1, method = "NA"), "width.*?integer")  # negative width
    expect_error(replace_outliers(x, 10, method = "NA"), "width.*?half")  # width >= length(x)/2
    expect_error(replace_outliers(x, 3, method = "NA", t0 = -1), "t0.*?integer")  # negative t0

    ## haldes all NA
    x <- rep(NA_real_, 10)
    expect_error(replace_outliers(x, 3), "x.*?numeric") ## x is all NA

    ## handles all same values
    expect_equal(replace_outliers(c(1, 1, 1), 1), rep(1, 3))
})
